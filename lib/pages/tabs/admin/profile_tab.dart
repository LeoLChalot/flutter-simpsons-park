...
class _ProfileTabState extends State<ProfileTab> {
              ...
              return AlertDialog(
                ...
                content: Form(
                  ...
                  child: Column(
                    ...
                    children: <Widget>[
                      const Text('Veuillez entrer votre mot de passe actuel pour continuer.'),
                ...
                actions: <Widget>[
                  ...
                  ElevatedButton(
                    onPressed: isLoadingReauth ? null : () async {
            ...
            builder: (context, setStateDialog) {
              return AlertDialog(
                ...
                actions: <Widget>[
                  // Bouton d'annulation
                  TextButton(
                    onPressed: isLoadingNewPassword ? null : () {
                      Navigator.of(dialogContext).pop(false);
                    },
                    child: Text('Annuler'),
                  ),
                  // Bouton de confirmation
                  ElevatedButton(
                    onPressed: isLoadingNewPassword ? null : () async {
    ...
    if (passwordChanged == true) {
      if (mounted) {
        await showDialog<void>(
          ...
          builder: (BuildContext alertContext) { // Contexte différent pour ce dialogue
            return AlertDialog(
              title: Text('Changement de mot de passe réussi'),
              content: Text('Vous allez être déconnecté. Merci de vous reconnecter avec votre nouveau mot de passe.'),
              actions: <Widget>[
                // Bouton OK
                TextButton(
                  child: const Text('OK'),
        ...
        // Après que l'utilisateur a cliqué "OK" sur la popup d'information,
        // s'assurer à nouveau que le widget ProfileTab est monté avant de déconnecter et naviguer.
        if (mounted) {
          ...
          try {
            ...
            // Gérer l'erreur de déconnexion si nécessaire, bien que la redirection
            // vers accessForm gère le cas où l'utilisateur est déconnecté.
          } finally {
            // Pas besoin de setState(() => _isLoading = false) ici car on navigue ailleurs.
          }
        }
      }
    }
            ...
            child: ListView(
              children: <Widget>[
                Center(
                  child: CircleAvatar(
                    ...
                    child: (user.photoURL == null || user.photoURL!.isEmpty)
                        ? Icon(Icons.person, size: 50, color: simpsonsTheme.colorScheme.onPrimaryContainer)
                        : null,
                  ),
                ),
                    ...
                    child: Column(
                      ...
                      children: [
                        if (user.displayName != null && user.displayName!.isNotEmpty) ...[
                          Text(
                            user.displayName!,
                            style: simpsonsTheme.textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.bold),
                          ),
                          const SizedBox(height: 8),
                        ],
                        _buildUserInfoRow(context: context, icon: Icons.email_outlined, label: 'Email', value: user.email ?? 'Non fourni'),
                        const Divider(height: 20),
                        _buildUserInfoRow(context: context, icon: Icons.vpn_key_outlined, label: 'User ID (UID)', value: user.uid),
                        if (user.metadata.creationTime != null) ...[
                          const Divider(height: 20),
                          _buildUserInfoRow(context: context, icon: Icons.date_range_outlined, label: 'Compte créé le', value: MaterialLocalizations.of(context).formatMediumDate(user.metadata.creationTime!)),
                        ],
                        if (user.metadata.lastSignInTime != null) ...[
                          const Divider(height: 20),
                          _buildUserInfoRow(context: context, icon: Icons.login_outlined, label: 'Dernière connexion', value: MaterialLocalizations.of(context).formatFullDate(user.metadata.lastSignInTime!)),
                        ],
                      ],
                    ),
                  ),
                ...
                ElevatedButton.icon(
                  ...
                  onPressed: _isLoading ? null : () => _showChangePasswordDialog(context, user),
                ),
                const SizedBox(height: 16),
                ElevatedButton.icon(
                  ...
                  onPressed: _isLoading ? null : () async {
                    ...
                    // Pas besoin de setState(() => _isLoading = false) si la navigation réussit
                  },
  ...
  Widget _buildUserInfoRow({required BuildContext context, required IconData icon, required String label, required String value}) {
    return Padding(
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(label, style: simpsonsTheme.textTheme.bodySmall?.copyWith(color: simpsonsTheme.colorScheme.onSurfaceVariant)),
